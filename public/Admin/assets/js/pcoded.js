    ...
    pc_link_click[i].addEventListener('click', function (event) {
      ...
      if (parentLi.classList.contains('pc-trigger')) {
        ...
        for (var t = 0; t < tc.length; t++) {
          var c = tc[t];
          c.classList.remove('pc-trigger');
          var cSubmenu = c.children[1];
          if (cSubmenu) {
            slideUp(cSubmenu, 200);
            window.setTimeout(() => {
              cSubmenu.removeAttribute('style');
...
let slideUp = (target, duration = 0) => {
  ...
  // Ensure the target has a style property to manipulate
  if (target.style) {
    target.style.transitionProperty = 'height, margin, padding';
    target.style.transitionDuration = duration + 'ms';
    target.style.boxSizing = 'border-box';
    target.style.height = target.offsetHeight + 'px';
    target.offsetHeight;  // Force reflow to make the height transition work
    ...
    console.error('slideUp function: Target element does not have a style property.');
  }
};
